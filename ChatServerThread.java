...
public class ChatServerThread extends Thread {
	...
	private Vector<ChatMessage> unreadMessage = null;
	
	public ChatServerThread(ChatServer _server, Socket _socket, ServerForm window) {
		super();
		server = _server;
		socket = _socket;
		ID     = socket.getPort();
		username = ID+"";
		unreadMessage = new Vector<ChatMessage>();
	}
			...
			try {
				...
				if (modeString.equals("CHAT")) {
					...
					while(!unreadMessage.isEmpty()){
						prepareToSend(unreadMessage.remove(0));
					}
				} 
